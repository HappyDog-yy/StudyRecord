...
function mergeSort(arr){
    ...
    function sort(arr,left,right){
        if(left<right){
            ...
            const leftArr:number = sort(arr,left,mid);
            const rightArr:number = sort(arr,mid+1,right);
...
```ts
// 基数排序
function basicSort(arr:number[]):number[]{
    // 获取数字num的digit位数字,个位为0，十位为1
    function getDigitNum(num:number,digit:number){
    ...
    for(let i:number=0;i<maxDigit;i++){
        // 初始化10个桶，分别代表0-9，每个桶是一个数组
        let buckets:number[] = new Array(10).fill(0).map(()=>[]);
        for(let k=0;k<arr.length;k++){
            // 获取该元素的第i位数字，看看能放在哪个桶中
            const bucketIndex:number = getDigitNum(arr[k],i);
